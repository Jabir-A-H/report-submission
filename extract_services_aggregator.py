...
AGG_FILE = "services/report_aggregator.py"

with open(AGG_FILE, "r", encoding="utf-8") as f:
    content = f.read()

# Make sure we don't accidentally insert it multiple times if run twice
if "def generate_aggregated_report_data" not in content:
    wrapper_logic = """
...
def dict_to_agg_report(data):
    ...
    return agg
"""
    content += wrapper_logic
    with open(AGG_FILE, "w", encoding="utf-8") as f:
        f.write(content)
    print("SUCCESS: Injected Pandas wrapper into report_aggregator.py")
