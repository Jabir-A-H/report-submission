...
*   **City Report Overhaul (Rebuilt from Scratch)**: 
    *   **The Vision**: Move away from a "giant table of numbers" toward a **Visual Dashboard**. 
    *   **Simple & Beautiful**: Focus on key performance indicators (KPIs) through summary cards, followed by organized, readable data sections.
    *   **Persistent Correction Layer (Overrides)**: 
        *   **The Logic**: The system will calculate totals from zone reports but will check the `city_report_override` table for a match. If an entry exists, it uses the **Override Value** instead of the sum.
        *   **Use Case**: As you requested, this allows you to fix a city-wide total (e.g., adding a missing field) without ever modifying the original zone-level records.
        *   **UI Indication**: Any "overridden" value will be clearly marked (e.g., highlighted in a different color) so admins can see at a glance what was manually adjusted.
*   **Exports**: 
