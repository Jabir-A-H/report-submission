...
Migrate the existing Flask-based monolith to a modern, mobile-first **Next.js + Supabase** stack. This plan ensures $0 hosting costs while delivering a high-performance **Adaptive Matrix** UI/UX designed specifically for your high-density data requirements (250+ fields).

> [!NOTE]
> We analyzed `reportdu.com` and `dinlipi.online` for architectural understanding. Our final design will NOT match their visuals but will use a custom, more professional design language (Shadcn/UI based).
...
*   **Admin Approval**: **Verified**: We will use your existing **`people` table** (which I have confirmed has the `active` column). Supabase Auth will handle secure sessions, linked via `user_id` to your `people` records.
...
*   **City Report**: 
    *   **Logic**: Maintain the current summation logic (aggregating all zone reports for the specific month/year).
    *   **UI**: Professional, readable data sections (KPIs and detailed grids).
*   **City Report Override Overhaul (Rebuilt from Scratch)**: 
    *   **The Vision**: Remake the *Override Feature itself* into something simple and beautiful.
    *   **Simple & Beautiful UX**: Instead of a clunky separate page, allow admins to initiate a "Correction" directly from the City Report view.
    *   **Persistent Correction Layer**:
        *   The logic will check the `city_report_override` table. If an entry exists, it uses the **Override Value** instead of the calculated sum.
        *   **Use Case**: Fixes city-wide totals without ever modifying the original zone-level records.
        *   **UI Indication**: Any "overridden" value will be clearly highlighted so admins can see it at a glance.
*   **Exports**: 
