...
5. [ ] **Complete Supabase Browser Client Memoization Consistency**: Align remaining client-side instantiation patterns by memoizing browser client usage in `src/app/report/page.tsx` and standardizing the creation path in `src/app/report/[section]/page.tsx`.
6. [ ] **Mobile Touch Ergonomics (WCAG Lints)**: Expand touch target padding on nested table rows in `auto-save-field.tsx` (`min-h-[44px] min-w-[44px]`).
