...
7. [x] **Overview Report (`/report`) URL Parameter Standardization & Filter Synchronization**: Standardized all report period query parameters across `/report` and navigation links (`type` / `report_type`) to clean English (`monthly`, `quarterly`, `half_yearly`, `yearly`, `annual`), eliminating URL percent-encoding bloat (`%E0%A6%AE%E0%A6%BE...`). Decoupled `/report` UI filter states from database queries so that selecting dropdown options does not trigger premature queries; clicking **"ফিল্টার করুন"** pushes English query parameters to `searchParams`, accurately updating both URL state and reported aggregate data simultaneously.
8. [ ] **Mobile Touch Ergonomics (WCAG Lints)**: Expand touch target padding on nested table rows in `auto-save-field.tsx` (`min-h-[44px] min-w-[44px]`).
