...
- **Desktop Top Bar (`Navbar`)**: When logged in as an `admin` or `superadmin` (`profile.role`), the header stays visible across all `/admin/*` routes and displays the 4 core admin links (**ড্যাশবোর্ড**, **জমাকৃত রিপোর্ট**, **এডিট রিপোর্ট**, **ব্যবস্থাপনা**) right inside the top bar alongside `UserDropdown`.
- **Mobile Bottom Bar (`BottomNav`)**: When `isAdmin` is true, the bottom bar automatically transforms into a 5-tab admin layout (`grid-cols-5`) right at the bottom of the screen: **ড্যাশবোর্ড** (`/admin`), **জমাকৃত** (`/admin/reports`), **এডিট** (`/admin/city-report`), **ব্যবস্থাপনা** (`/admin/management`), and **প্রোফাইল** (toggles slide-up panel with Theme, Language, Help, and Logout).
...
| `/admin/city-report` | Aggregated city-wide report + inline override editing (titled "এডিট রিপোর্ট", override mode always active, comparative dual value display) |
...
- **Inline City Overrides (`/admin/city-report`)**: Titled **এডিট রিপোর্ট** (`Edit Report`), this page stays permanently in override mode (`isEditing = true` with static `⚡ ওভাররাইড মোড সক্রিয়` badge). Admins click any aggregated numeric cell to adjust values or delete existing corrections via `CorrectionButton`. Overrides are saved to `city_report_override` (`UNIQUE NULLS NOT DISTINCT` constraint) and rendered as a comparative dual pill `[ <s>computedValue</s> newValue ]` alongside the original aggregated value. This is the **only** page where city-level data can be corrected. The override page uses `view_city_*_agg` views (which already apply `zone_type != 'city'` exclusions) and then applies overrides via `city_report_override` on top.
...
- `src/app/admin/city-report/page.tsx` — Aggregated city report viewer titled "এডিট রিপোর্ট" (`Edit Report`) with `city_report_override` inline editing (override mode always active, clean amber badge highlighting without strike-through old values, and tap-to-open change panel behavior via `customTrigger`).
- `src/app/admin/management/page.tsx` — Merged Users + Zones management page (`নতুন জোন` button, compact mobile KPI ribbon, collapsible filters, and click-outside `if unchanged` modal closure).
...
- `docs/ADR/` — Formal Architecture Decision Records (`001` through `011`).
