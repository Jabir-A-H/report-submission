...
def generate_excel_file(reports, report_type):
    ...
    for report_index, (report, report_orm) in enumerate(zip(serialized_reports, reports)):
        # Add zone separator
        if report_index > 0:
            data.extend([[""] * 7, [""] * 7])  # Empty rows

        # Zone Header
        zone_name = report_orm.zone.name if hasattr(report_orm, "zone") and report_orm.zone else ""
        if zone_name:
        ...
        if personal:
            data.append(["বিভাগ", "ব্যক্তিগত উদ্যোগে তা'লীমুল কুরআন", "কতজন শিখাচ্ছেন", "কতজনকে শিখাচ্ছেন", "ওয়ালামাকে দাওয়াত", "সহযোগী হয়েছেন", "সক্রিয় সহযোগী", "কর্মী হয়েছেন", "রুকন হয়েছেন"])
            for category in PERSONAL_CATEGORIES:
                ...
                data.append([
                    ...
                    _safe_get(p, "became_shohojogi"), _safe_get(p, "became_sokrio_shohojogi"),
                    _safe_get(p, "became_kormi"), _safe_get(p, "became_rukon")
                ])
            data.append([""] * 9)
        ...
        if meetings:
            data.append(["বিভাগ", "বৈঠকসমূহ", "মহানগরী কতটি", "মহানগরী গড় উপস্থিতি", "থানা কতটি", "থানা গড় উপস্থিতি", "ওয়ার্ড কতটি", "ওয়ার্ড গড় উপস্থিতি", "মন্তব্য"])
            for category in MEETING_CATEGORIES:
                ...
                data.append([
                    ...
                    _safe_get(m, "ward_count"), _safe_get(m, "ward_avg_attendance"),
                    m.get("comments") or ""
                ])
            data.append([""] * 9)
    ...
    # Create DataFrame with flexible column structure (max columns is now 9 for meetings/personal)
    headers = ["বিভাগ", "বিষয়", "কলাম ১", "কলাম ২", "কলাম ৩", "কলাম ৪", "কলাম ৫", "কলাম ৬", "কলাম ৭"]
    df = pd.DataFrame(data)
    # Handle column names dynamically if data rows vary in width
    num_cols = df.shape[1]
